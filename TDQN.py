...
class TDQN:
    ...
    def save_model(self, filepath):
        """
        Save the trained model with all necessary components to resume training or testing
        
        INPUTS: - filepath: Path where the model will be saved
        ...
        checkpoint = {
            ...
            'observation_space': self.observationSpace,
            'action_space': self.actionSpace,
            'replay_memory': self.replayMemory.memory,
    ...
    def load_model(self, filepath):
        ...
        INPUTS: - filepath: Path to the saved model
        ...
        try:
            ...
            # Verify observation and action spaces match
            if self.observationSpace != checkpoint.get('observation_space'):
                print(f"Warning: Loaded model has different observation space: {checkpoint.get('observation_space')} vs {self.observationSpace}")
            
            if self.actionSpace != checkpoint.get('action_space'):
                print(f"Warning: Loaded model has different action space: {checkpoint.get('action_space')} vs {self.actionSpace}")
            
            # Load replay memory if available
            if 'replay_memory' in checkpoint:
                self.replayMemory.memory = checkpoint['replay_memory']
                print(f"Loaded replay memory with {len(self.replayMemory.memory)} experiences")
            
            # Load iteration counter if available
            ...
            if 'timestamp' in checkpoint:
                print(f"Model was saved on: {checkpoint['timestamp']}")
    ...
    def safe_training(self, trainingEnv, trainingParameters=[], verbose=False, 
                      rendering=False, plotTraining=False, showPerformance=False):
        """
        GOAL: A production-safe wrapper for the training method with better error handling
        
        INPUTS: Same as training method
        
        OUTPUTS: trainingEnv and status dictionary
        """
        status = {
            "success": False,
            "error": None,
        ...
        try:
            # Call the regular training method
            result = self.training(trainingEnv, trainingParameters, 
                                  verbose, rendering, plotTraining, showPerformance)
            
            status["success"] = True
            status["completed_episodes"] = trainingParameters[0]
            status["training_time"] = (datetime.datetime.now() - start_time).total_seconds()
            
            return result, status
            
        except KeyboardInterrupt:
            # Handle intentional interruptions
            status["error"] = "Training interrupted by user"
            status["completed_episodes"] = self.iterations // len(trainingEnv.data)  # Approximate
            status["training_time"] = (datetime.datetime.now() - start_time).total_seconds()
            print("\nTraining interrupted by user. Model state was preserved.")
            return trainingEnv, status
