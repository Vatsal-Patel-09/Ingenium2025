...
from dataDownloader import CSVToDataframe
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, marketSymbol, startingDate, endingDate, money, stateLength=30,
        ...
        """
        self.marketSymbol = marketSymbol  # Make sure this line exists
        self.startingDate = startingDate
        self.endingDate = endingDate

        # CASE 1: Fictive stock generation
        print(self.startingDate, self.endingDate)
        if(self.marketSymbol in fictiveStocks):
            stockGeneration = StockGenerator()
            if(self.marketSymbol == 'LINEARUP'):
                self.data = stockGeneration.linearUp(self.startingDate, self.endingDate)
            elif(self.marketSymbol == 'LINEARDOWN'):
                self.data = stockGeneration.linearDown(self.startingDate, self.endingDate)
            elif(self.marketSymbol == 'SINUSOIDAL'):
                self.data = stockGeneration.sinusoidal(self.startingDate, self.endingDate)
            else:
                self.data = stockGeneration.triangle(self.startingDate, self.endingDate)
        ...
        # CASE 2: Real stock loading
            ...
            csvName = "".join(['Data/', self.marketSymbol, '_', self.startingDate, '_', self.endingDate])
            ...
            # Otherwise, download the stock market data from Yahoo Finance and save it in the database
                ...
                try:
                    self.data = downloader1.getDailyData(self.marketSymbol, self.startingDate, self.endingDate)
                except:
                    self.data = downloader2.getDailyData(self.marketSymbol, self.startingDate, self.endingDate)

                if saving == True:
                    csvConverter.dataframeToCSV(csvName, self.data)

            self.data = self._ensure_data_columns(self.data)
        ...
        self.state = [self.data['Close'][0:stateLength].values.tolist(),
                      self.data['Low'][0:stateLength].values.tolist(),
                      self.data['High'][0:stateLength].values.tolist(),
                      self.data['Volume'][0:stateLength].values.tolist(),
                      [0]]
        self.reward = 0.
        self.done = 0
    ...
    def _ensure_data_columns(self, data):
        """
        Ensure the DataFrame has all required columns and proper structure.
        Creates synthetic data if needed or fills missing columns.
        """
        required_columns = ['Close', 'Open', 'High', 'Low', 'Volume']
        
        # If data is completely empty, create synthetic data
        if data is None or data.empty:
            print(f"WARNING: Creating synthetic data for {self.marketSymbol}")
            # Create date range for synthetic data
            try:
                dates = pd.date_range(self.startingDate, self.endingDate)
            except:
                # Fallback if date parsing fails
                print(f"Date parsing failed, using 1000 days of synthetic data")
                dates = pd.date_range(start='2015-01-01', periods=1000)
                
            # Create synthetic dataframe with proper columns
            data = pd.DataFrame(index=dates)
            base_price = 100.0
            # Generate simple trending data
            data['Close'] = np.linspace(base_price, base_price*2, len(dates))
            data['Open'] = data['Close'] * (0.99 + 0.02 * np.random.random(len(dates)))
            data['High'] = data['Close'] * (1.01 + 0.02 * np.random.random(len(dates)))
            data['Low'] = data['Close'] * (0.98 - 0.02 * np.random.random(len(dates)))
            data['Volume'] = 1000000 * (0.5 + np.random.random(len(dates)))
            print(f"Synthetic data created with {len(dates)} trading days")
            return data
        
        # Make sure the index is datetime
        if not isinstance(data.index, pd.DatetimeIndex):
            print("WARNING: Converting index to DatetimeIndex")
            try:
                # Try to convert the index to datetime
                data.index = pd.to_datetime(data.index)
            except:
                # If conversion fails, create new index
                print("Index conversion failed, creating new index")
                data.index = pd.date_range(start='2015-01-01', periods=len(data))
        
        # Check and handle column format issues
        ...
        if missing_columns:
            ...
            # Try to map columns from different formats
            for missing_col in missing_columns.copy():
                for alt_name, std_name in column_mappings.items():
                    if std_name == missing_col and alt_name in data.columns:
                        print(f"Using {alt_name} as {std_name}")
                        data[std_name] = data[alt_name]
                        missing_columns.remove(missing_col)
                        break
            
            # Create any remaining missing columns based on available data
            if 'Close' in missing_columns:
                if any(col in data.columns for col in ['Open', 'High', 'Low']):
                    # Use another price column
                    for col in ['High', 'Low', 'Open']:
                        if col in data.columns:
                            print(f"Deriving Close from {col}")
                            data['Close'] = data[col]
                            missing_columns.remove('Close')
                            break
                else:
                    # Create a default price series
                    print("Creating synthetic Close prices")
                    data['Close'] = np.linspace(100, 200, len(data))
                    missing_columns.remove('Close')
            
            # Create any other missing columns based on Close
            if 'Close' in data.columns:
                ...
                if 'Volume' in missing_columns:
                    data['Volume'] = 1000000  # Constant volume
        ...
        # Final check that all required columns exist
        for col in required_columns:
            if col not in data.columns:
                print(f"WARNING: Still missing {col}, creating default values")
                if col == 'Volume':
                    data[col] = 1000000
                else:
                    data[col] = 100.0  # Default price value
        
        print(f"Data columns after processing: {data.columns.tolist()}")
